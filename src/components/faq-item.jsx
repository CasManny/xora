    ...
    <div className="relative z-2 mb-16">
      ...
      >
        <div className="flex-1">
          <div className="small-compact mb-1.5 text-p3 max-lg:hidden text-left">
            {index < 10 ? "0" : ""}
            {index}
          </div>
          <div
            className={clsx(
              "h6 text-p4 text-left transition-colors duration-500 max-md:flex max-md:min-h-20 max-md:items-center",
      ...
      <SlideDown>
        {activeId === faq.id && (
          <div className="body-3 px-7 py-3.5 text-left">{faq.answer}</div>
        )}
      </SlideDown>
      <div
        className={clsx(
            "g5 -bottom-7 -top-7 left-0 absolute right-0 -z-1 rounded-3xl opacity-0 transition-opacity duration-500",
            active && "opacity-100"
        )}
      >
